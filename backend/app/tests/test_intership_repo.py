...
def test_create_internship(db_session):
    repo = InternshipRepo(db_session)
    ...
    created_internship = repo.create(internship)
...
def test_get_internship_by_id(db_session):
    ...
    created_internship = repo.create(internship)
    fetched_internship = repo.get_by_id(created_internship.internship_id)
...
def test_update_internship(db_session):
    ...
    repo.update(created_internship.internship_id, updated_data)
    updated_internship = repo.get_by_id(created_internship.internship_id)
    assert updated_internship.status == 2

def test_create_internship_enrollment(db_session):
    repo = InternshipEnrollmentRepo(db_session)
...
def test_get_internship_enrollment_by_student_id(db_session):
    ...
    enrollments = repo.get_by_student_id(1)
